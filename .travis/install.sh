...
if [ $2 = "2" ]; then
    ...
    docker exec --user root ndts /bin/sh -c 'export DEBIAN_FRONTEND=noninteractive; apt-get -qq update; apt-get -qq install -y python-enum34 python-pytango python-tz; apt-get -qq install -y nxsconfigserver-db; sleep 10'
else
...
if [ $2 = "2" ]; then
    ...
    docker exec --user root ndts /bin/sh -c 'export DEBIAN_FRONTEND=noninteractive; apt-get -qq update; apt-get install -y  python-nxsconfigserver python-nxswriter python-nxstools'
else
    echo "install sardana, taurus and nexdatas"
    docker exec --user root ndts /bin/sh -c 'export DEBIAN_FRONTEND=noninteractive; apt-get -qq update; apt-get install -y  python3-nxsconfigserver python3-nxswriter python3-nxstools'
fi
if [ $? -ne "0" ]
then
    exit -1
...
if [ $2 = "2" ]; then
    ...
    docker exec --user root ndts /bin/sh -c 'pwd; ls; python setup.py -q install || true'
else
